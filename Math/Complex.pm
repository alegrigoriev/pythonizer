...
sub _theta {
    ...
    if    ($theta >   pi()) { $theta -= pi2 }	# SNOOPYJC
    elsif ($theta <= -pi()) { $theta += pi2 }	# SNOOPYJC
    return $theta;	# SNOOPYJC
}
...
sub _stringify_polar {
	...
	if ($format{polar_pretty_print} && $t) {
	    ...
	    for $a (2..9) {
		$b = $t * $a / pi;
		$b =~ s/\.0$//;		# SNOOPYJC: python formats floats always with '.0' at the end - strip it
		if ($b =~ /^-?\d+$/) {
