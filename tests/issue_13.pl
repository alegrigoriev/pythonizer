...
assert($t =~ /At the tone, the time will be [A-Z][a-z][a-z] [A-Z][a-z][a-z] \d+ \d\d:\d\d:\d\d \d\d\d\d/);
assert(scalar localtime() =~ /[A-Z][a-z][a-z] [A-Z][a-z][a-z] \d+ \d\d:\d\d:\d\d \d\d\d\d/);
#say STDOUT scalar @z;
