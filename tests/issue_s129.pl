...
#END{print"1..$C\n$M"}
my($C,@M);sub ok{$C++;$M[$C]=$_[0]?1:"fail on line ".(caller)[2]."\n"}
...
# confuses pythonizer and just gives an error on translation: eval { case 1 { ok(0) }; ok(0) } || ok(1);
...
switch (3) {
    ...
    # confuses pythonizer and we don't handle this anyhow: eval { case __ <= 1 || __ > 2	{ ok(0) } } || ok(1);
	case __ <= 2 		{ ok(0) };
	case __ <= 3		{ ok(1) };
}
...
# pythonizer doesn't handle this: for ('a'..'c','1')
for ('a', 'b', 'c', '1')
{
	switch ($_) {
		# SELF
