    ...
    if(exists $line_locals{$lno}) {                     # issue s100
        for my $local (reverse @{$line_locals{$lno}}) {
            ...
            if($sigil eq '*') {         # We appended the ones we need to the name
                ...
                if($pyname =~ /_set_signal\(/) {                # issue s336
                    $pyname =~ s/_set_signal/signal.signal/;    # issue s336
                }                                               # issue s336
    ...
    for($i = 1; $i<=$#ValClass; $i++) {
        ...
        if($ValClass[$i] =~ /[ashG]/) {
            ...
            last;
                ...
                $ValClass[$i+2] eq '"') {       # issue s277: Special handling for local $SIG{XXX}
            my $real_name = "$ValPy[$i](" . &::unquote_string($ValPy[$i+2]) . ")";  # issue s277
            push @locals, $real_name;           # issue s277
            last;                               # issue s277
        }
    }
...
sub prepare_local
{
    ...
    if($sigil eq '$') {
        ...
        decode_hash($quote);
    } elsif($sigil eq '&') {        # issue s284
        decode_bare(substr($quote,1));   # issue s284
    } elsif($sigil =~ /[A-Za-z_]/) {
        decode_bare($quote);
...
# Note that file scoped locals are not handled here (they are treated like "my" vars).
{
    ...
    foreach my $lno (keys %line_locals) {
        for(my $i = 0; $i < scalar(@{$line_locals{$lno}}); $i++) {
            ...
            if($sigil eq '*') {
                my $id = substr($quote,1);
                # issue s284: In this case $id is 'main::test_sub' and $NameMap contains an entry for main.test_sub with '&'
                $line_locals{$lno}->[$i] .= ',';
                my $id_dot = $id;               # issue s284
                $id_dot =~ tr/::/./s;           # issue s284
                $id_dot =~ tr/'/./;             # issue s284
                # issue s284 if(exists $NameMap{$id}) {
                # issue s284     for my $sig (keys %{$NameMap{$id}}) {
                if(exists $NameMap{$id_dot}) {      # issue s284
                    for my $sig (keys %{$NameMap{$id_dot}}) {   # issue s284
                        # issue s284 next if($sig eq '&');
   ...
   while( defined $source && $source ne ''){    # issue s13
      ...
      if( $s eq '#'  ){
         ...
         if((2 <= $#ValClass && $ValClass[0] eq 'h' && $ValClass[1] eq '=' && $ValClass[2] eq '(') ||
            ...
            while(defined ($source=Pythonizer::getline(2))) {
                push @lines_of_statement, $source;          # issue s278
            ...
            push @lines_of_statement, $source if defined $source;     # issue s278
         }
     ...
     # SNOOPYJC last if( $source=~/^\s*[;{}]\s*(#.*)?$/); # single closing statement symnol on the line.
      ...
      if(index('$@%&*', $s) >= 0 && substr($source,1,1) eq '{' &&
         ...
         (substr($source,1,3) =~ /^{.}/ ||                       # issue s341
          (index('$@%&*"\'', substr($source,2,1)) < 0 &&
           substr($source,2) !~ m'^\s+[$@%&*"\']'))              # issue s341
        ) {     # issue 43 - Look for ${...} and delete the brackets
     ...
     if( $tno > 2 && $ValClass[$tno-1] eq 'i' and $ValPerl[$tno-2] eq '{' ) {   # issue 13
          ...
          if( $tno==0 && looks_like_anon_hash_def($source)) {   # issue test coverage
             ...
             # issue 82 }elsif( length($source)==1 ){
              ...
              ;                                 # issue s213
                  ...
                  $ValClass[$tno-1] ne 'a' &&                   # issue s338
                  $ValClass[$tno-1] ne '@' &&                   # issue s339
                  $ValClass[$tno-1] ne '%' &&                   # issue s339
                  $ValClass[$tno-1] ne '$' &&                   # issue s339
      ...
      # issue 35 }elsif( $ValClass[$tno-1] eq ')' || $source=~/^.\s*#/ || index($source,'}',1) == -1){
         ...
         # issue s262 } elsif($s eq '{' && $ValClass[$tno-1] eq 'f' && semicolon_in_block($source)) {     # issue s39
         } elsif($s eq '{' && $ValClass[$tno-1] eq 'f' && !safe_for_lambda($source)) {     # issue s39, issue s262
     ...
     # issue 23 if( $source=~/(^\d+(?:[.e]\d+)?)/  ){
         ...
         } elsif($tno != 0 && $ValClass[$tno-1] eq 'P') {   # issue s284: '::' => 'P'
             popup();               # issue s284
             $tno--;                # issue s284
             $w = 'main::' . $w;    # issue s284
         }
         ...
         if( exists($keyword_tr{$w}) ){
            ...
            if($w eq 'warn' || $w eq 'die') {       # issue s288: Keep track if we call warn or die
                my $cs = cur_sub();                 # issue s288
                $SpecialVarsUsed{$w}{$cs} = 1;      # issue s288
            }                                       # issue s288
         }
         ...
         if( exists($TokenType{$w}) ){
            ...
            if($tno != 0 && (($ValPerl[$tno-1] eq '{' && $source =~ /^[a-z0-9]+}/) ||   # issue 89: keyword in a hash like $hash{delete} or $hash{q}
                ...
                if($pd < 0 || substr($subname, 0, $pd) eq cur_package()) {  # issue s243
                    ...
                    my $cs = cur_sub();
                    $Pythonizer::initialized{$cs}{$ValPy[$tno]} = 1;            # issue s281
                }
          ...
          if(index($ValPy[$tno], "\n") >= 0 && substr($ValPy[$tno],0,1) eq 'f' && $ValPy[$tno] !~ /^f"""/) { # issue 39 - multi-line string
                  ...
                  if( ($w eq 'm' || $w eq 'qr') && ($tno>=1 && $ValPerl[$tno-1] eq 'split') ||
                      ...
                      if( length($modifier) > 1 ){
                        ...
                        # issue s286 $quoted_regex='re.compile('.put_regex_in_quotes($arg1, $delim, $original_regex, x_flag($modifier))."$modifier)";   # issue 111, issue s80
                         $quoted_regex='re.compile('.put_regex_in_quotes($arg1, $delim, $original_regex, $modifier)."$modifier)";   # issue 111, issue s80, issue s286
                      }else{
                        # No modifier
                        $quoted_regex=put_regex_in_quotes($arg1, $delim, $original_regex, $modifier);       # issue 111, issue s80, issue s286
                      }
                  ...
                  # issue s52    $ValPy[$tno]='r'.$quoted_regex; #  double quotes neeed to be escaped just in case
                     ...
                     $cut=0;
                     my $arg1_modifier;
                     # issue s344 (undef,$groups_are_present)=is_regex($arg1,1);         # issue s131
                     ($arg1_modifier,$groups_are_present)=is_regex($arg1,1);         # issue s131, issue s344
                     ...
                     if( length($modifier)>0 || length($arg1_modifier) > 0){        # issue s344
                        #this is regex
                        ...
                        # issue s344 $ValPy[$tno]='str.replace('.$quoted_regex.','.$quoted_regex.',1)';
                        $ValPy[$tno]='.replace('.$quoted_regex.','.put_regex_in_quotes($arg2, $delim, $original_regex2, 0, 1).',1)';     # issue s344
                     }
                     ...
                     # issue s286 my $x_flag = x_flag($modifier);                     # issue s80
                     my $original_modifier = $modifier;                     # issue s80, issue s286
                     ($arg1, $modifier) = build_in_qr_flags($arg1, $modifier);          # issue s3
                     # issue s286 $ValPy[$tno]='re.compile('.put_regex_in_quotes($arg1, $delim, $original_regex, $x_flag).$modifier.')';       # SNOOPYJC, issue 111
                     $ValPy[$tno]='re.compile('.put_regex_in_quotes($arg1, $delim, $original_regex, $original_modifier).$modifier.')';       # SNOOPYJC, issue 111, issue s286
...
#                         }
                  ...
                  # we can emulate it with split function, althouth wq is mainly compile time.
                   ...
                   if( $ValPerl[0] eq 'use' && $ValPerl[1] ne 'constant' ){     # SNOOPYJC
                      ...
                      if($::replace_run) {                      # issue s294
                          my @words = split(/ /, $python);      # issue s294
                          for my $word (@words) {               # issue s294
                             # issue s349 $word = &::unquote_string(replace_run("'" . $word . "'")); # issue s294: Replaces the word in @words
                             $word = &::unquote_string(replace_run(escape_quotes($word))); # issue s294: Replaces the word in @words, issue s349: The word can have a quote in it!!
                          }                                     # issue s294
                          $python = join(' ', @words);          # issue s294
                      }                                         # issue s294
...
# issue s215               $Pythonizer::VarType{$ValPy[$tno]}{$cs} = &Pythonizer::merge_types($ValPy[$tno], $cs, $type) if(defined $type);   # issue s173, issue s215
               ...
               #$ValPerl[$tno]=$ValPy[$tno]=$s; # issue 50
            } elsif($tno != 0 && $ValClass[$tno-1] eq '*' && $ValPerl[$tno-1] ne 'x' &&         # issue s358
                !$had_space && ($tno-1 == 0 || $ValClass[$tno-2] !~ /[sdfi)]/)) {  # issue s76
            ...
            if( $ValPy[$tno] eq 'SIG' ) {              # issue 81 - implement signals
               ...
               if($tno == 0 || ($tno == 1 && $ValClass[0] eq 't')) {   # at start of line like $SIG{ALRM} = sub { die "timeout"; };
                   ...
                   if($source =~ /\{\s*__DIE__/) {
                       $ValClass[$tno] = 's';
# issue s292                       if($source =~ /(?:Carp::)?confess/) {
# issue s292                           $ValPy[$tno] = $DIE_TRACEBACK;
# issue s292                           $ValPy[$tno] = "$PERLLIB.$DIE_TRACEBACK" if($::import_perllib);
# issue s292                           $source =~ s/\{\s*__DIE__\s*\}\s*=.*$/=1;/;
# issue s292                       } elsif($source =~ /;\s*$/ || $source =~ /;\s+#/) {  # issue s277
# issue s292                           $ValPy[$tno] = $DIE_TRACEBACK;
# issue s292                           $ValPy[$tno] = "$PERLLIB.$DIE_TRACEBACK" if($::import_perllib);
# issue s292                           $source =~ s/\{\s*__DIE__\s*\}\s*=.*$/=0;/;
# issue s292                           $source =~ s/\{\s*__DIE__\s*\}\s*;/=0;/;  # issue ddts: Handle "local $SIG{__DIE__};"
# issue s292                       } else {         # issue s277
# issue s292                           $ValPy[$tno] = $DIE_TRACEBACK;       # issue s277
# issue s292                           $ValPy[$tno] = "$PERLLIB.$DIE_TRACEBACK" if($::import_perllib);      # issue s277
# issue s292                           $source =~ s/\{\s*__DIE__\s*\}\s*=/=/;      # issue s277
# issue s292                       }
                       $ValPy[$tno] = 'SIG_DIE_HANDLER'; # issue s292
                       $ValPy[$tno] = "$PERLLIB.SIG_DIE_HANDLER" if($::import_perllib);  # issue s292
                       $ValPerl[$tno] = '$' . $ValPy[$tno];          # issue s292: add '$' so we don't get 'erllib.SIG_DIE_HANDLER'
                       $ValPerl[$tno] =~ s/[.]/::/g;                 # issue s292
                       $source =~ s/\{\s*__DIE__\s*\}\s*=/=/;        # issue s292
                       $source =~ s/\{\s*__DIE__\s*\}\s*;/=undef;/;  # issue s292: Handle "local $SIG{__DIE__};"
                       #  issue s292 $ValPerl[$tno] = '$' . $ValPy[$tno];          # issue ddts: add '$' so we don't get 'erllib.TRACEBACK'
                       #  issue s292 $ValPerl[$tno] =~ s/[.]/::/g;                 # issue ddts
                   } elsif($source =~ /\{\s*__WARN__/) {
                       $ValClass[$tno] = 's';
                       # issue s288 $ValPerl[$tno] = $ValPy[$tno] = 'warnings.showwarning';
                       $ValPy[$tno] = 'SIG_WARN_HANDLER'; # issue s288
                       $ValPy[$tno] = "$PERLLIB.SIG_WARN_HANDLER" if($::import_perllib);  # issue s288
                       $ValPerl[$tno] = '$' . $ValPy[$tno];          # issue s277: add '$' so we don't get 'erllib.SIG_WARN_HANDLER'
                       $ValPerl[$tno] =~ s/[.]/::/g;                 # issue s277
                       $source =~ s/\{\s*__WARN__\s*\}\s*=/=/;
                       $source =~ s/\{\s*__WARN__\s*\}\s*;/=undef;/;  # issue s277: Handle "local $SIG{__WARN__};"
                   } else {
                       # Change to signal.signal(SIG, RHS);
                       $ValPy[$tno] = 'signal.signal';
                       $source =~ s/=\s*['"]DEFAULT['"]/=_DFL/;
                       $source =~ s/=\s*['"]IGNORE['"]/=_IGN/;
                       if($source =~ /\{\s*([A-Z_]+)\s*\}\s*=\s*(.*)\s(?:if|unless)\b/) {   # issue s336: statement modifier
                           $source =~ s/\{\s*([A-Z_]+)\s*\}\s*=\s*(.*)(\s(?:if|unless).*);/($1, $2)$3;/;    # issue s336
                       } else {
                           $source =~ s/\{\s*([A-Z_]+)\s*\}\s*=\s*(.*);/($1, $2);/;
                           $source =~ s/\{\s*([A-Z_]+)\s*\}\s*;/($1, _IGN);/;       # issue ddts: Handle "local $SIG{CHLD};"
                       }
                       if($source !~ /\([^,)], (?:_IGN|_DFL)\)/) {  # issue s336
                           $ValPy[$tno] = '_set_signal';            # issue s336
                       }                                            # issue s336
                   ...
                   if($source =~ /\{\s*__DIE__/) {              # issue s277
                       $ValClass[$tno] = 's';
                       # issue s292 $ValPy[$tno] = $DIE_TRACEBACK;       # issue s277
                       # issue s292 $ValPy[$tno] = "$PERLLIB.$DIE_TRACEBACK" if($::import_perllib);      # issue s277
                       $ValPy[$tno] = 'SIG_DIE_HANDLER';       # issue s292
                       $ValPy[$tno] = "$PERLLIB.SIG_DIE_HANDLER" if($::import_perllib);      # issue s292
                       $source =~ s/\{\s*__DIE__\s*\}//;      # issue s277
                       $ValPerl[$tno] = '$' . $ValPy[$tno];          # issue s277: add '$' so we don't get 'erllib.TRACEBACK'
                       $ValPerl[$tno] =~ s/[.]/::/g;                 # issue s277
                   } elsif($source =~ /\{\s*__WARN__/) {             # issue s277
                       $ValClass[$tno] = 's';
                       $ValPy[$tno] = 'SIG_WARN_HANDLER'; # issue s288
                       $ValPy[$tno] = "$PERLLIB.SIG_WARN_HANDLER" if($::import_perllib);  # issue s288
                       $ValPerl[$tno] = '$' . $ValPy[$tno];          # issue s277: add '$' so we don't get 'erllib.SIG_WARN_HANDLER'
                       $ValPerl[$tno] =~ s/[.]/::/g;                 # issue s277
                       $source =~ s/\{\s*__WARN__\s*\}//;            # issue s277
                   }
                   $source =~ tr/{}/()/;
                }
                if($::debug >= 3 && $Pythonizer::PassNo!=&Pythonizer::PASS_0) {
         ...
         if($s2 eq '' || $s2 =~ /\s/) {               # issue ws after sigil
            $source = get_rest_of_variable_name($source,0);
            $s2=substr($source,1,1);        # issue s338
         }
         ...
         } elsif($s2 =~ /[+-]/) {   # issue s338: @+ or @- is the array version of %+ or %-
              $ValPy[$tno]=$SPECIAL_VAR{$s2};
              $cut=2;
              $ValType[$tno]="X";
              my $vn = substr($source,0,2);                     # issue s338
              my $svar = $vn;                                   # issue s338
              my $nxc = length($source) >= 2 ? substr($source,2,1) : '';                    # issue s338
              $svar = '@' . substr($svar,1) if($nxc eq '[');    # issue s338
              $svar = '%' . substr($svar,1) if($nxc eq '{');    # issue s338
              if($svar eq '%+' || $svar eq '%-') {          # issue s338
                  $ValPy[$tno] = "$DEFAULT_MATCH.groupdict()";        # issue s338: %+ is different than @+
              } else {
                  $ValPy[$tno] = "[$ValPy[$tno](_i) for _i in range($DEFAULT_MATCH.lastindex+1)]";  # issue s338
              }
              my $cs = cur_sub();
              $SpecialVarsUsed{$svar}{$cs} = 1;     # issue s338: Capture the actual var referenced, e.g. $-[0] => @-
              $ValPerl[$tno]=$vn;                   # issue s338
              $ValClass[$tno]='a'; # issue s338: array
         }else{
            $cut=1;
         }
      }elsif( $s eq '%' ){
         ...
         if( substr($source,1)=~/^(\:?\:?\'?[_a-zA-Z]\w*((?:(?:\:\:)|\')[_a-zA-Z]\w*)*)/ && # old perl used ' for ::
            ...
            if($tno != 0 && $ValClass[$tno-1] eq 'f' && $ValPerl[$tno-1] eq 'each') {        # issue s305
             ...
             $ValPy[$tno] = $ValPerl[$tno] = '%=';
             $cut = 2;
         } elsif($s2 =~ /[+-]/) {   # issue s342: %+ or %-
              $ValPy[$tno]=$SPECIAL_VAR{$s2};
              $cut=2;
              $ValType[$tno]="X";
              my $vn = substr($source,0,2);                     # issue s342
              my $svar = $vn;                                   # issue s342
              $ValPy[$tno] = "$DEFAULT_MATCH.groupdict()";      # issue s342: %+
              my $cs = cur_sub();
              $SpecialVarsUsed{$svar}{$cs} = 1;     # issue s342
              $ValPerl[$tno]=$vn;                   # issue s342
              $ValClass[$tno]='h'; # issue s342: hash
         }else{
           $cut=1;
         }
         ...
         if( substr($source,1)=~/^(\:?\:?\'?[_a-zA-Z]\w*((?:(?:\:\:)|\')[_a-zA-Z]\w*)*)/){
            if(ampersand_is_sub_sigil()) {      # issue s152: distinguish & from &Sub
                ...
                $ValType[$tno] = '&';       # issue s351: Distinguish &sub from plain sub for sort
                $ValPerl[$tno]=$1;
                $ValPy[$tno]=$1;
                $ValPy[$tno]=~tr/:/./s;
                $ValPy[$tno]=~tr/'/./s;             # SNOOPYJC
        ...
        $tno--;             # issue 50 - no need to keep arrow operator in python
         ...
         if($trigram eq '<=>') {                # issue 66
           ...
           # issue 62 if( $tno==2 && $ValClass[0] eq 'a' && $ValClass[1] eq '='){
               if( $tno>=2 && $ValClass[$tno-2] eq 'a' && $ValClass[$tno-1] eq '='){    # issue 62: handle "my @a=<FH>;" and "chomp(my @a=<FH>);"
                   if(length($fh)==0){         # issue 66: Pure diamond
                     ...
                     # issue s345 $ValPy[$tno]="$fh.readlines()";
                     my $escaped = escape_keywords($fh);              # issue s345
                     $ValPy[$tno]="$escaped.readlines()";             # issue s345
                  }
                   ...
                   if(length($fh)==0){         # issue 66
                       ...
                       if($rl =~ /readline_full/) {     # issue s288
                           $ValPy[$tno]="$rl($escaped, '$fh')";   # issue 66, issue s25, issue s288
                       } else {
               ...
               $::Pyf{'_fileinput_next()'} = 1;     # issue bootstrap - this line is so that _fileinput_next() is replaced with perllib.fileinput_next()
                       ...
                       my $rl = select_readline();                      # issue 66
                       if($rl =~ /readline_full/) {     # issue s288
                           $ValPy[$tno]="$rl(sys.stdin, 'stdin')";      # issue 66: support $/, issue s188: Remove the extra ()
                       } else {
                       ...
                       my $escaped = escape_keywords($fh);              # issue s25
                       if($rl =~ /readline_full/) {     # issue s288
                           $ValPy[$tno]="$rl($escaped, '$fh')";           # issue 66: support $/, issue s25, issue s288
                       } else {
           ...
           # issue 62 if( $tno==2 && $ValClass[0] eq 'a' && $ValClass[1] eq '='){
               if( $tno>=2 && $ValClass[$tno-2] eq 'a' && $ValClass[$tno-1] eq '='){    # issue 62: handle "my @a=<FH>;" and "chomp(my @a=<FH>);"
                   # issue s345 $ValPy[$tno]="$1.readlines()";
                   my $escaped = escape_keywords($1);              # issue s345
                   $ValPy[$tno]="$escaped.readlines()";            # issue s345
               }elsif($ValClass[0] eq 'c' and $ValPerl[0] eq 'while') { # issue 66
                   ...
                   if($rl =~ /readline_full/) {     # issue s288
                       $ValPy[$tno]="$rl($escaped, '$mapped_name')";                # issue 66: Support $/, $., issue s25, issue s288
                   } else {
                       $ValPy[$tno]="$rl($escaped)";                # issue 66: Support $/, $., issue s25
                   }
               }else{
      ...
      if($ValClass[$tno] =~ /[ahsG]/) {         # SNOOPYJC: Handle globals in packages
          ...
          if($Pythonizer::PassNo != &Pythonizer::PASS_0) {      # issue s316
              ...
              $Pythonizer::VarType{$ValPy[$tno]}{$cs} = &Pythonizer::merge_types($ValPy[$tno], $cs, $type, $tno) if(defined $type);   # issue s173, issue s215, issue s356
   ...
   if($tno > 0) {                                       # issue 94
        ...
        } elsif($ValClass[0] eq 'a' && $ValPerl[0] eq '@ISA' && $ValClass[1] eq '=' && cur_sub() eq '__main__') { # issue s339
            handle_ISA_assignment(1);               # issue s339
...
# Select either the simple or the full version of readline depending on if $. and $/ are used or not
{
    ...
    $rl = "_readline_full" if(exists $SpecialVarsUsed{'$/'} || exists $SpecialVarsUsed{'$.'} ||
                              exists $SpecialVarsUsed{warn} || exists $SpecialVarsUsed{die});   # issue s288
    $::Pyf{$rl} = 1; 
    if($::import_perllib) {
...
my $rc=-1;
   ...
   if(($s2 eq '$' || $s2 eq '%') && substr($source,2,1) =~ /[\w:']/) {   # issue 50: $$ is a special var, but not $$a or $$: or $$'
       $specials = '!';
   } elsif(defined $ValType[$tno] && $ValType[$tno] eq '${' &&
           ($s2 eq '"' || $s2 eq "'") && substr($source,2,1) =~ /[\w:']/) {     # issue s341: ${ "Date... } is NOT $"
       $specials = '!';
   }
   if( $s2 eq '.'  ){
      # file line number
      ...
      if( ($k=index($name,'::')) > -1 ){
         ...
         if( $s2 eq '_' ){
            ...
            if( $source=~/^(._\s*\[\s*(\d+)\s*\])/  ){
               ...
               # issue s359: Can't do this on LHS!!  $ValPy[$tno]="$PERL_ARG_ARRAY".'.get('.$2.')' if $::autovivification;   # issue s359
...
my (@temp,$sym,$prev_sym,$i,$modifier,$meta_no);
   ...
   for( $i=0; $i<@temp; $i++ ){
      ...
      if( $prev_sym ne '\\' && $sym eq '(' && !($temp[$i+1] eq '?' && $temp[$i+2] eq ':')){    # issue s131: (?:...) is not capturing
         ...
         return($modifier,1);
      }elsif($prev_sym eq '$' && substr($myregex,$i) =~ /^(?:{|(\w+))/) {    # issue GPT regex: assume any variable can contain groups!!  issue s352: Handle ${...} like $...
      ...
      # issue s230 }elsif( $prev_sym ne '\\' && index('.*+()[]?^$|',$sym)>=-1 ){
        ...
        $meta_no++;
      }elsif(  $prev_sym eq '\\' && lc($sym)=~/[bsdwSDWpP]/){       # issue s344: Add pP
         $meta_no++;
      }elsif(  $prev_sym eq '\\' && index('abfntrg0\\xNuU', $sym)>-1) {     # issue s344: Can't use .replace for these
         $meta_no++;                                                        # issue s344
      }
      $prev_sym=$sym;
   }#for
   ...
   if((exists $SpecialVarsUsed{'@-'} && exists $SpecialVarsUsed{'@-'}{$cs}) ||
      (exists $SpecialVarsUsed{'$&'} && exists $SpecialVarsUsed{'$&'}{$cs}) ||  # issue s341
      (exists $SpecialVarsUsed{'$`'} && exists $SpecialVarsUsed{'$`'}{$cs}) ||  # issue s341
      (exists $SpecialVarsUsed{'$\''} && exists $SpecialVarsUsed{'$\''}{$cs}) ||  # issue s341
      (exists $SpecialVarsUsed{'%-'} && exists $SpecialVarsUsed{'%-'}{$cs}) ||  # issue s342
      (exists $SpecialVarsUsed{'%+'} && exists $SpecialVarsUsed{'%+'}{$cs}) ||  # issue s342
      (exists $SpecialVarsUsed{'@+'} && exists $SpecialVarsUsed{'@+'}{$cs})) {
      ;
   } else {
      if ($modifier eq 'r') { $modifier = ''; } # issue s344: It's a string, not a regex
   }
   say STDERR "is_regex($myregex,$first_s) = ($modifier, 0)" if($::debug >= 5 && $Pythonizer::PassNo != &Pythonizer::PASS_0);
   return($modifier,0);     # issue 11
}
...
#
   ...
   if( length($modifier) > 1 ){
      #regex with modifiers
      # issue s286 $quoted_regex='re.compile('.put_regex_in_quotes($myregex, $delim, $original_regex, x_flag($modifier), $s_rhs).$modifier.')';  # issue 111, issue s80, issue s131
      $quoted_regex='re.compile('.put_regex_in_quotes($myregex, $delim, $original_regex, $modifier, $s_rhs).$modifier.')';  # issue 111, issue s80, issue s131
      ...
      # No modifier
      $quoted_regex=put_regex_in_quotes($myregex, $delim, $original_regex, $modifier, $s_rhs);      # issue 111, issue s131, issue s286
   ...
   if( length($modifier)>0 ){
      ...
      # this is a string
      # issue s344 $ValClass[$tno]="'";
      $ValClass[$tno]='"';
      # issue s230 return '.find('.escape_quotes(escape_non_printables($myregex,0)).')';
      if( ($tno>=1 && $ValClass[$tno-1] eq 'p') ||
          ($tno>=2 && $ValClass[$tno-1] eq '(' && $ValClass[$tno-2] eq 'f' && $ValPerl[$tno-2] eq '__expand')   # issue s344
        ){        # issue s230
...
my  $outer_delim;
    ...
    # issue s359 }elsif(index($quote,"'")==-1 && index($quote,'{')==-1 && index($quote,"\n")==-1     # issue 53, multi-line here
    }elsif(index($quote,"'")==-1 && index($quote,'{')==-1 && index($quote,'[')==-1 && index($quote,"\n")==-1     # issue 53, multi-line here, issue s359
   ...
   while( $k > -1  ){
      ...
      if($sig eq '$') {                 # issue 47
        ...
        #}
         ...
         if($prev eq '@') {     # this was like @$var or @{$var}
            ...
            # issue s355 if($next_c eq '{' || $next_c eq '[' || $et =~ /[SFIR]/) {       # issue s117
            $close = '';                # issue s355
            if($::autovivification && $sig eq '$' && $ValPy[$tno] =~ /^$PERL_ARG_ARRAY\[/) {    # issue s359
                $ValPy[$tno] =~ s/\[/.get(/;        # issue s359
                $ValPy[$tno] =~ s/\]/)/;            # issue s359
            }                                       # issue s359
            if($et =~ /[SFIR]/) {       # issue s117
                $result.=$ValPy[$tno]; # copy string provided by decode_scalar. ValPy[$tno] changes if Perl contained :: like in $::debug
            } elsif($sig eq '$' && ($next_c eq '{' || $next_c eq '[' || $next_c eq '-')) {       # issue s355
                $result.='_bn(' . $ValPy[$tno];       # issue s355: convert None to '', else grab the value as is
                $close = ')';                         # issue s355
            } else {                                  # issue s117
      ...
      if($sig eq '$') {                 # issue 47
          ...
          while($ind = extract_bracketed($quote2, $brackets, '')) {        # issue 53, issue 98, issue s314
             ...
             for(my $i = 0; $i < length($ind); $i++) {  # issue 53: change hash ref {...} to use .get(...) instead
                 ...
                 if($c eq '{') {        # issue 53
                     ...
                     # issue s359 if($next_ch eq '{') {                          # issue s107
                     if($next_ch eq '{' || $next_ch eq '[') {       # issue s107, issue s359
                     ...
                     my $next_ch = substr($quote, $ind_cut, 1);     # issue s359
                     if(!$::autovivification || $next_ch eq '[' || $next_ch eq '{') {   # issue s359
                        ...
                        if(substr($ind,$i+1,1) eq "'") {               # issue s359 Array with string key
                            $ind = substr($ind,0,$i).'.get('.substr($ind,$i+2,$l-($i+3)).",'')".substr($ind,$l+1); # issue s123: Strip off the quotes
                        } else {                                   # issue s123
                            $ind = substr($ind,0,$i).'.get('.substr($ind,$i+1,$l-($i+1)).",'')".substr($ind,$l+1);  # issue s359: splice in the call to get
                        }
                    }
                     ...
                     if(substr($var,1,1) eq '{') {      # SNOOPYJC: like ${i}
                         # issue s359 $l = matching_curly_br($var, 1);
                         $l = matching_curly_br($ind, $i);  # issue s359: .get( moves things over!
                         if($l > 0) {                   # SNOOPYJC: Eat both the '{' and the '}'
                             # issue s359 substr($ind,$l+1,1) = '';
                             substr($ind,$l,1) = '';        # issue s359
                             substr($ind,$i+1,1) = '';
                             $var = substr($ind,$i);
                         }
          ...
          #issue s355 if($prev eq '@') {
          #issue s355 $result.='))';     # Close the join/map operator inserted above
          #issue s355 }
          $result .= $close;            # issue s355 (also handle the join/map close here)
          $prev = '';
      }
...
my $delim=$_[1];        # issue 111
my $original_regex=$_[2]; # issue 111
# issue s286 my $x_flag=$_[3];       # issue s80
my $modifier=$_[3];       # issue s80, issue s286
my $s_rhs = (scalar(@_) > 4 ? $_[4] : 0);       # issue bootstrap: Is this on the RHS of a s/// ?
   if($::debug > 4 && $Pythonizer::PassNo != &Pythonizer::PASS_0) {
       say STDERR "put_regex_in_quotes($string, $delim, $original_regex, $modifier)";
   }
   my $x_flag = x_flag($modifier);      # issue s286
   ...
   if($delim ne "'") {  # issue 111
       ...
       $ValPy[$tno] = perl_regex_to_python($ValPy[$tno], $modifier, 1) unless($s_rhs);          # issue 111, issue bootstrap, issue s81, issue s286
   ...
   return 'r'.escape_quotes(perl_regex_to_python($string, $modifier));   # SNOOPYJC
}
...
# Convert a perl regex to a python regex
{
    ...
    if(m_flag($regex, $modifier)) {                             # issue s286
        $regex = fixup_carat_in_m_flag_regex($regex);           # issue s286
    }                                                           # issue s286
...
# Remove some oddities from the generated code to make it easier to read/understand
{
    ...
    # issue s359: Change perllib.Array(perllib.Array(...)) to perllib.Array(...)
    $pos = -1;
    while(($pos = index($line, 'perllib.Array(perllib.Array(', $pos+1)) > 0) {
        next if(in_string($line, $pos));
        my $end_pos = python_matching_paren($line, $pos+13);
        my $end_pos1 = python_matching_paren($line, $pos+27);    # Make sure this isn't like "perllib.Array(perllib.Array(...)...)"
        if($end_pos1+1 == $end_pos && $end_pos > $pos) {
            substr($line,$end_pos,1) = '';
            substr($line,$pos,14) = '';
        }
    ...
    # issue s359: change perllib.list_of_n(perllib.Array(...), N) -to- perllib.list_of_n(..., N)
    if(($pos = index($line, 'perllib.list_of_n(perllib.Array(')) > 0) {
        if(!in_string($line, $pos)) {
            my $end_pos = python_matching_paren($line, $pos+17);
            my $end_pos1 = python_matching_paren($line, $pos+31);
            if(substr($line, $end_pos1, ($end_pos+1)-$end_pos1) =~ /^\),\s*\d+\)/) {
                substr($line,$end_pos1,1) = '';
                substr($line,$pos+18,14) = '';
            }
        }
    }
...
#
   for($i=0; $i<@_;$i++ ){
      if( scalar(@PythonCode) >$MAX_CHUNKS  ){
         ...
         logme('T',"The number of generated chunks exceeed $MAX_CHUNKS");
    ...
    for(my $i=0; $i<scalar(@ids); $i++) {
       ...
       if($is_package_name == 2) {
          if($PYTHON_RESERVED_SET{$id} && !exists $PYTHON_PACKAGES_SET{$id}) {       # issue s200 
           ...
           # issue s284 if(scalar(@ids) == 1 || !exists $PYTHON_PACKAGES_SET{$id}) {           # issue s200: Don't change sys. to sys_., don't change perllib.stat to perllib.stat_
           if(scalar(@ids) == 1 || !exists $PYTHON_PACKAGES_SET{$ids[0]}) {           # issue s200: Don't change sys. to sys_., don't change perllib.stat to perllib.stat_, issue s284
               $id = $id.'_';
           }
       } elsif(substr($id,0,1) =~ /\d/) {   # issue ddts: @1234 => _1234
           $id = '_'.$id;
       # issue s300 } elsif($i == 0 && substr($id,0,1) eq '_' && substr($id,0,5) ne '__END' && $id !~ /^_f\d/ && exists $Pyf{$id}) {    # issue s126
       } elsif($i == $#ids && substr($id,0,1) eq '_' && substr($id,0,5) ne '__END' && $id !~ /^_f\d/ && exists $Pyf{$id}) {    # issue s126, issue s300
           $id = $id.'_';                   # issue s126
       }
...
sub safe_for_lambda          # issue s39, issue s262
# Return 1 if everything in {...} block like in a map/grep function is safe for a python lambda function
{
    my $source = shift;

    my $end_br = matching_curly_br($source, 0);
    return 0 if($end_br < 0);                           # issue s78: assume any multi-line block needs to be pulled out
    $end_br = length($source) if $end_br < 0;
    $source = substr($source,0,$end_br);
    return 0 if(index($source,';') >= 0);        # No good if multiple statements
    return 0 if(index($source,'++') >= 0);       # No good if incr
    return 0 if(index($source,'--') >= 0);       # No good if decr
    my $peq = 0;
    while(($peq = index($source, '=', $peq)) != -1) {   # No good if assignment, but != and == are ok
        if(index('=>~', substr($source, $peq+1, 1)) != -1) {    # ==, =>, =~ are all good, except:
            return 0 if substr($source, $peq) =~ /=~\s*(?:s|tr)/;  # =~ s/// -or- =~ tr/// is no good!
            $peq++;
        } else {
            return 0 if $peq == 0 || index('!<>', substr($source, $peq-1, 1)) == -1 # bad unless !=, <=, >=
        }
    } continue {
        $peq++;
    }
...
sub m_flag                      # issue s286
# Return 1 if this regex modifier has an x flag, return 2 if it has 2 x flags
{
    my $regex = $_[0];
    my $modifier = $_[1];

    #say STDERR "m_flag($modifier)";
    return 1 if(index($modifier, 're.M') >= 0);
    return 1 if $regex =~ /\(\?\^?[a-ln-z]*m[a-ln-z]*/;       # Look for (?m -or- (?^m -or like- (?smi -or like- (?^smi
    return 0;
}

sub fixup_carat_in_m_flag_regex     # issue s286
#  Fixup '^' in regex with m flag to be the same as what perl does - see https://github.com/snoopyjc/pythonizer/issues/286 for details 
{
    my $regex = shift;
    return $regex if $regex !~ /\^/;        # quick exit if we don't have a carat char at all in the regex
    my $in_char_class = 0;
    my @result = ();
    for(my $i = 0; $i < length($regex); $i++) {
        my $c = substr($regex, $i, 1);
        if($in_char_class) {
            $in_char_class = 0 if $c eq ']';
            push @result, $c;
            next;
        } elsif($c eq '[') {
            $in_char_class = 1;
    ...
    say STDERR "fixup_carat_in_m_flag_regex($regex) = $result" if $::debug >= 5;
    return $result;
}
    ...
    for(my $i = 0; $i <= $#ValClass; $i++) {
        if($ValClass[$i] eq 'i' && ($Pythonizer::LocalSub{$ValPy[$i]} || ($i != 0 && $ValClass[$i-1] eq 'D'))) {
           ...
           if(defined &Pythonizer::get_sub_attribute_at($i, 'out_parameters')) {    # issue s241
               ...
               $TokenStr=join('',@ValClass);     # issue s339
               for(my $arg = 0; ;$arg++) {
