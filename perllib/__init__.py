# perllib module for pythonizer, generated by makelib.py on Tue Feb  8 16:22:33 2022
#
...
def fcntl(fh, func, scalar):
    ...
    try:
        return fc_py.fcntl(fh, func, scalar)
    except Exception as e:
        if TRACEBACK:
            traceback.print_exc()
        raise
...
def fdopen(fh, fd, mode):
    ...
    if isinstance(fd, int):
        fd = f'={fd}'
    return open_dynamic(_open_mode_string(mode) + '&' + fd)
...
def ioctl(fh, func, scalar):
    ...
    try:
        return fc_py.ioctl(fh, func, scalar)
    except Exception as e:
        if TRACEBACK:
            traceback.print_exc()
        raise
