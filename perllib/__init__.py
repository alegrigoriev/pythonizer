# perllib module for pythonizer, generated by makelib.py on Fri Jan 28 18:53:56 2022
import sys,os,re,fileinput,subprocess,collections.abc,warnings,inspect,itertools,signal,traceback,io,tempfile,calendar,types,random,dataclasses,builtins
...
def binmode(file,mode='b',encoding=None,errors=None):
    """Handle binmode"""
    global OS_ERROR, TRACEBACK, AUTODIE
    try:
        file.flush()
...
def carp(*args):
    ...
    print(shortmess(*args, skip=1), end='', file=sys.stderr)
...
def cluck(*args):
    ...
    print(longmess(*args, skip=1), end='', file=sys.stderr)
...
def confess(*args):
    ...
    raise Die(longmess(*args, skip=1))

def croak(*args):
    """Error with no backtrace"""
    raise Die(shortmess(*args, skip=1))
...
def has_setgid(path):        # -g
    ...
    if hasattr(path, 'mode'):
        return (path.mode & st_py.S_ISGID) != 0
...
def has_setuid(path):        # -u
    if not path:
        return False
    if hasattr(path, 'mode'):
        return (path.mode & st_py.S_ISUID) != 0
...
def has_sticky(path):        # -k
    if not path:
        return False
    if hasattr(path, 'mode'):
        return (path.mode & st_py.S_ISVTX) != 0
...
def is_block_special(path):        # -b
    if not path:
        return False
    if hasattr(path, 'mode'):
        return st_py.S_ISBLK(path.mode)
...
def is_char_special(path):        # -c
    if not path:
        return False
    if hasattr(path, 'mode'):
        return st_py.S_ISCHR(path.mode)
...
def is_dir(path):        # -d
    if not path:
        return False
    if hasattr(path, 'mode'):
        return st_py.S_ISDIR(path.mode)
...
def is_file(path):        # -f
    if not path:
        return False
    if hasattr(path, 'mode'):
        return st_py.S_ISREG(path.mode)
...
def is_link(path):        # -l
    if not path:
        return False
    if hasattr(path, 'mode'):
        return st_py.S_ISLNK(path.mode)
...
def is_pipe(path):        # -p
    if not path:
        return False
    if hasattr(path, 'mode'):
        return st_py.S_ISFIFO(path.mode)
...
def is_socket(path):        # -S
    if not path:
        return False
    if hasattr(path, 'mode'):
        return st_py.S_ISSOCK(path.mode)
...
def open_dynamic(file,mode=None,checked=True):
...
def stat_cando(self, mode, eff):
    ...
    if uid == 0 or (sys.platform == 'cygwin' and _ingroup(544, eff)):    # Root
        ...
        if (self.mode & 0o111) or st_py.S_ISDIR(self.mode):
            return True
        return False
    if self.uid == uid:
        if (self.mode & mode):
